	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				// var Discord Discord
				// var p discordstructs.Notification

				// p.Content = fmt.Sprintf("BybitWs Read error, closing connection: %v", err)
				// DiscordInterface.SendNotification(&p)

				log.Panicf("BybitWs Read error, closing connection: %v", err)
				return
			}
